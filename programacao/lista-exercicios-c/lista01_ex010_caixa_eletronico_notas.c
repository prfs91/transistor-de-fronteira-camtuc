#include <stdio.h>   // Biblioteca padrão: permite usar printf (mostrar) e scanf (ler).
#include <stdlib.h>  // Biblioteca de utilidades (boa prática incluir).

/*
  Arquivo: lista01_ex010_caixa_eletronico_notas.c

  Objetivo:
    Receber do usuário um valor inteiro em reais (ex.: 378)
    e calcular quantas notas/moedas são necessárias da forma mais
    "ótima": sempre usando primeiro as notas de maior valor.

  Conjunto de cédulas usadas no Brasil (atual):
    - Notas: 200, 100, 50, 20, 10, 5, 2 reais
    - Moeda: 1 real

  Estratégia:
    Algoritmo "guloso" (greedy):
      Para cada nota, começando da maior:
        - Ver quantas notas cabem no valor restante.
        - Subtrair do restante.
      Como as denominações brasileiras são canônicas,
      o método guloso gera sempre a solução ótima.
*/

int main(void) {

    int valor_saque;   // Armazena o valor total digitado pelo usuário.
